...
var Modal=function(){
    ...
    _createClass(Modal,[{key:"getInstance",value:function getInstance(){return this}},
    ...
    {key:"setupEventHandlers",value:function setupEventHandlers(){
        this.handleOverlayClickBound=this.handleOverlayClick.bind(this);
        this.handleModalCloseClickBound=this.handleModalCloseClick.bind(this);
        if(Modal._count===1){document.addEventListener("click",this.handleTriggerClick)}
        this.$overlay[0].addEventListener("click",this.handleOverlayClickBound);
        this.$el[0].addEventListener("click",this.handleModalCloseClickBound)}
    },
    {key:"removeEventHandlers",value:function removeEventHandlers(){
        if(Modal._count===0){document.removeEventListener("click",this.handleTriggerClick)}
        this.$overlay[0].removeEventListener("click",this.handleOverlayClickBound);
        this.$el[0].removeEventListener("click",this.handleModalCloseClickBound)}
    },
    {key:"handleTriggerClick",value:function handleTriggerClick(e){
        var $trigger=$(e.target).closest(".modal-trigger");
        if(e.target&&$trigger.length){var modalId=$trigger[0].getAttribute("href");
        if(modalId){modalId=modalId.slice(1)}
        else{modalId=$trigger[0].getAttribute("data-target")}
        var modalInstance=document.getElementById(modalId).M_Modal;
        if(modalInstance){modalInstance.open($trigger)}e.preventDefault()}}
    },
    {key:"handleOverlayClick",value:function handleOverlayClick(){
        if(this.options.dismissible){this.close()}}
    },
    {key:"handleModalCloseClick",value:function handleModalCloseClick(e){
        var $closeTrigger=$(e.target).closest(".modal-close");
        if(e.target&&$closeTrigger.length){this.close()}}
    },
    {key:"handleKeydown",value:function handleKeydown(e){
        if(e.keyCode===27&&this.options.dismissible){this.close()}
    }
    },
    {key:"animateIn",value:function animateIn(){
        var _this=this;$.extend(this.$el[0].style,{display:"block",opacity:0});
        $.extend(this.$overlay[0].style,{display:"block",opacity:0});
        Vel(this.$overlay[0],{opacity:this.options.opacity},{duration:this.options.inDuration,queue:false,ease:"easeOutCubic"});
        var enterVelocityOptions={
            duration:this.options.inDuration,queue:false,ease:"easeOutCubic",complete:function(){
                if(typeof _this.options.ready==="function"){_this.options.ready.call(_this,_this.$el,_this.openingTrigger)}}
        };
        if(this.$el[0].classList.contains("bottom-sheet")){Vel(this.$el[0],{bottom:0,opacity:1},enterVelocityOptions)}
        else{Vel.hook(this.$el[0],"scaleX",.7);
        this.$el[0].style.top=this.options.startingTop;Vel(this.$el[0],{top:this.options.endingTop,opacity:1,scaleX:1},enterVelocityOptions)}}
    },
    {key:"animateOut",value:function animateOut(){
        var _this2=this;
        Vel(this.$overlay[0],{opacity:0},{duration:this.options.outDuration,queue:false,ease:"easeOutQuart"});
        var exitVelocityOptions={
               duration:this.options.outDuration,queue:false,ease:"easeOutCubic",complete:function(){
               _this2.$el[0].style.display="none";
                if(typeof _this2.options.complete==="function"){_this2.options.complete.call(_this2,_this2.$el)}
                
                 $('.'+_this2.$overlay[0].className).remove();
